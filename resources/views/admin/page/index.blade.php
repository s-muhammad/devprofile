...
@section('main')
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 md:p-10">
        ...
        <div class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
            ...
            {{-- دکمه ایجاد صفحه جدید (کد فعال‌سازی) --}}
            {{-- <a href="{{ route('admin.page.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-blue-500/50 transition duration-300 transform hover:scale-[1.02] flex items-center gap-2">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8 overflow-x-auto">
            <table class="min-w-full text-right divide-y divide-gray-200">
                ...
                <tr class="bg-gray-100 text-gray-600 text-sm uppercase font-semibold tracking-wider">
                    ...
                    <th class="py-4 px-4">عنوان صفحه</th>
                    <th class="py-4 px-4">اسلاگ (URL)</th>
                ...
                @forelse($pages as $page)
                    <tr class="hover:bg-blue-50 transition-colors duration-200">
                        ...
                        <td class="py-4 px-4 text-gray-600">
                            <span class="bg-gray-200 text-gray-700 py-1 px-3 rounded-full text-sm font-mono">
                                /{{ $page->slug }}
                            </span>
                        </td>
                        {{-- <td class="py-4 px-4 text-gray-500 text-sm">...</td> --}}
                        <td class="py-4 px-4 text-center">
                            <div class="flex items-center justify-center space-x-3 space-x-reverse">
                                {{-- دکمه ویرایش (تنها دکمه فعال) --}}
                                <a href="{{ route('admin.page.edit', $page) }}" title="ویرایش صفحه" class="text-blue-600 hover:text-blue-800 p-2 rounded-full hover:bg-blue-100 transition duration-200">
                                    <i class="fas fa-edit text-lg"></i>
                                </a>
                                {{-- فرم حذف (کد غیرفعال) --}}
                                {{-- <form action="{{ route('admin.page.destroy', $page) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید؟');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="py-12 text-center text-gray-500 text-lg">
                            ...
                            <p>هیچ صفحه ای برای نمایش وجود ندارد.</p>
                        </td>
                    </tr>
                @endforelse
                </tbody>
