...
@section('main')
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 md:p-8">
        <h1 class="text-3xl font-extrabold text-gray-900 mb-6 border-b pb-2">👋 اقدامات سریع پنل</h1>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            <a href="{{ route('admin.service.create') }}" class="block transform transition-transform duration-300 hover:scale-[1.03]">
                <div class="bg-white rounded-xl shadow-xl p-6 md:p-8 border-r-4 border-blue-500 hover:bg-blue-50">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 mt-4">سرویس جدید</h3>
                    <p class="text-sm text-gray-500 mt-1">تعریف و ارائه خدمات و امکانات جدید.</p>
                </div>
            </a>
        </div>
        <h1 class="text-3xl font-extrabold text-gray-900 mb-6 border-b pb-2 mt-8">🔍 آمار فعالیت‌های اخیر</h1>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-right whitespace-nowrap">
                        ...
                        <tr class="text-gray-600 border-b-2 border-gray-200 text-sm uppercase tracking-wider bg-gray-50">
                            ...
                            <th class="py-3 px-4">تاریخ انتشار</th>
                        </tr>
                        </thead>
                        <tbody>
                        @forelse($blogs as $blog)
                            <tr class="border-b border-gray-100 hover:bg-purple-50/50 transition-colors duration-200">
                                ...
                                <td class="py-3 px-4 text-sm text-gray-500">{{ \Carbon\Carbon::parse($blog->created_at)->diffForHumans() }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 md:p-8">
                <h2 class="text-2xl font-bold text-gray-900 mb-6 flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <span>آخرین خدمات</span>
                    </div>
                    <a href="{{ route('admin.service.index') }}" class="text-sm font-medium text-red-600 hover:text-red-800 transition-colors">مشاهده همه &larr;</a>
                </h2>
                    ...
                    <table class="w-full text-right whitespace-nowrap">
                        ...
                        @forelse($services as $service)
                            <tr class="border-b border-gray-100 hover:bg-red-50/50 transition-colors duration-200">
                                <td class="py-3 px-4 text-gray-700">{{ $loop->iteration }}</td>
                                <td class="py-3 px-4 font-medium text-gray-900">{{ $service->title ?? 'کاربر ناشناس' }}</td>
                                <td class="py-3 px-4">
                                    @if(isset($service->image) && $service->image)
                                        <img src="{{ asset($service->image) }}" alt="{{ $service->title }}" class="w-12 h-10 object-cover rounded-md shadow">
                                    @else
                                        <span class="text-gray-400">-</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-6 text-gray-500">درخواست جدیدی برای نمایش وجود ندارد.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
